...
Вы открыли проект. Вкладка Инструменты > группа команд Управление > Порядок свойств
...
Благодаря использованию базовых проектов порядки свойств для 3D-размещения изделий уже доступны в новом проекте. Для условных обозначений порядки свойств отображаются, только если здесь в диалоговом окне или на самом условном обозначении сохранен определенный пользователем порядок свойств под именем.
...
Если необходимо вернуться к прежнему виду порядков свойств, удалите текст из поля ввода. Для этого нажмите кнопку  (Удалить).
...
Если данный флажок активирован, порядок свойств следует применять по умолчанию во всех других случаях использования 3D-размещения изделия / варианта символа.

Стандартно для кабельных соединений:

Доступны только точки определения соединений. Если этот флажок установлен и данный вариант символа используется для кабельных соединений, то этот порядок свойств используется по умолчанию. В [Настройках проекта](cablegui_r_allpoligeinpolig.htm) вы определяете, какой вариант символа используется при размещении точек определения соединений для кабельных соединений. Точки определения соединений для кабельных соединений размещаются при чертеже кабелей / экранирований или при перетаскивании мышью из навигатора кабелей или навигатора соединений в графическом редакторе.

* * *

Конфигурация свойств и настроек отображения

Сама конфигурация свойств с соответствующими настройками отображения выполняется для 3D-размещений изделий на вкладке Трехмерные функции, а для условных обозначений — на вкладке Условное обозначение, а также на вкладках Выводы устройства и Образ контакта. При этом доступны такие же возможности настройки, как и на вкладке [Отображение](devicetaggui_r_anzeige.htm) диалогового окна 'Свойства'.
