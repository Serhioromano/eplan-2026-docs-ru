...
}В этом примере классSimpleScriptсоздается через функциюMyFunction. Функция обозначается атрибутом[Start]. В случае выполнения сценария по командному пути Файл>Дополнительно> группа команд Расширения>Интерфейсы> группа команд Сценарии>Выполнитьпоявляется сообщениеMyFunction was called!.Аналогичный пример в Visual Basic.Net выглядит следующим обр.:Public Class SimpleScript
