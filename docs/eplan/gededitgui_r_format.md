...
* Из слоя: Текст находится в рамке, если это задано в настройках слоя, на котором находится текст. (Например, в [Управлении слоями](eplan/layermanager_d_ebenenverwaltung.md) для соответствующего слоя установлен флажок Рамка текста.)
...
Такое поведение соответствует выравниванию текстов свойств по центру, как описано в разделе [Свойства отображения: Размещенное свойство](eplan/devicetaggui_r_anzeigeeigenschaften.md#Blockzentriert).
...
По умолчанию для этих свойств установлен параметр "Из настройки проекта". В таком случае для свойства отображения даты / времени, которое размещается как специальный текст, используется формат, заданный в [настройках проекта](eplan/xessettingsgui_d_einstellungenprojektdatum.md).
