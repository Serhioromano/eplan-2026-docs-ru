...
Посредством операций [Импортировать тексты проекта в словарь](eplan/translatedbgui_h_projekttexteimportieren.md) и [Импортировать тексты изделий в словарь](eplan/translatedbgui_h_artikeltexteimportieren.md) цифры в текстах заменяются заполнителем. При импортировании текст проекта / изделия добавляется в словарь в качестве ключевого слова. Если текст проекта / изделия содержит цифру, в ключевом слове эта цифра будет заменена заполнителем.
...
Если знаки препинания . (точка), ! (восклицательный знак) или ? (вопросительный знак) встречаются в конце записи как знаки в конце предложения, они импортируются, только если в настройках пользователя (командный путь: Файл > Настройки > Пользователь > Перевод) на вкладке Словарь снят флажок Убрать знаки в конце предложения.

### Список отсутствующих слов

Посредством операции [Экспортировать список отсутствующих слов](eplan/translategui_h_fehlwortexport.md) цифры в текстах заменяются заполнителем.
...
Если система Eplan не находит подходящего ключевого слова, ищется ключевое слово, в котором на месте цифр находится заполнитель. В этом случае при переводе используются [ключевые слова с заполнителями](eplan/translatedbgui_k_eingabemoeglichkeit.md#Platzhalter).
