...
Вы открыли проект. Открыто диалоговое окно свойств для функции, символа или проекта. В поле Свойства выбрано свойство Свойство блока: Формат [n], а в столбце Значение нажата кнопка [...].
  ...
  * Выберите в диалоговом окне Формат в списке Доступные элементы формата группу свойств, и щелкните по символу "+", чтобы открыть группы свойств более низкого уровня. На нижнем слое выделите какую-либо группу и щелкните по кнопке  (Переместить направо).
  * Выделите в диалоговом окне Формат в списке Выбранные элементы формата имеющийся элемент формата и щелкните  (Обработать).
...
Вы открыли проект. Вкладка Предварительное планирование > группа команд Обработать > Конфигурировать определения сегмента. Нажмите в поле Формат отображения навигатора кнопку [...].
  ...
  * Выберите в диалоговом окне Формат в списке Доступные элементы формата группу свойств и нажмите на символ , чтобы открыть группы свойств более низкого уровня. На нижнем слое выделите какую-либо группу и щелкните по кнопке  (Переместить направо).
  * Выделите в диалоговом окне Формат в списке Выбранные элементы формата имеющийся элемент формата и щелкните  (Обработать).
...
Вы открыли проект. Вкладка Основные данные > группа команд Рамки / формы > раскрывающаяся кнопка Форма > [Открыть].
  ...
  * Вкладка Вставить > группа команд Текст > раскрывающаяся кнопка Формы > Текст-заполнитель. В диалоговом окне перейдите на вкладку Размещение, выберите параметр Форматированные / рассчитанные свойства и нажмите кнопку [...]. Выделите в списке Доступные элементы формата элемент формата и нажмите кнопку  (переместить вправо).
...
Выберите из раскрывающегося списка свойства, которые будут отображены в списке ниже: "Все", "Данные" или "Настройки". В списке выводятся все свойства выбранного типа, которым еще не было присвоено форматирование.
...
Эта настройка не влияет на блок идентификатора "Вид документа", структурные идентификаторы для вида документа отображаются всегда.
...
Если флажок установлен, то свойство не выводится, если оно имеет значение, заданное в находящемся ниже поле. Возможно использование [заполнителей](searchandreplacegui_k_platzhalter.htm) типа '*' и '?'.
...
Если флажок установлен, то свойство не выводится, если оно не имеет значения, заданного в находящемся ниже поле. Это означает, что свойство выводится только тогда, когда оно обладает заданным там значением. Возможно использование [заполнителей](searchandreplacegui_k_platzhalter.htm) типа '*' и '?'.
...
!!! note "Замечание:"
    ...
    Тексты для числовых значений свойств внутренне переводятся на все доступные языки диалогового окна. Если в качестве языка отображения вы выбрали язык, который недоступен какязык диалогового окна, текст будет отображен на языке, установленном как альтернативный язык.
...
Выберите из раскрывающегося списка используемую таблицу с текстами замены. Значения выбранного свойства сравниваются с текстами, сохраненными в таблице, и при необходимости заменяются.

  * Выберите запись "Использовать локальную таблицу", если таблицу с текстами замены нужно сохранить непосредственно в формате блока. Для этого нажмите кнопку […] и определите локальную таблицу с текстами замены в диалоговом окне Тексты замены.
  * Выберите запись "Использовать таблицу [n]", если вы хотите использовать таблицу, предварительно определенную в свойствах проекта или формы. Нажмите на кнопку […], чтобы узнать, была ли определена таблица для соответствующей записи в последующем диалоговом окне Тексты замены. Если это так, вы можете просмотреть определенные тексты замены без возможности их редактировать.
...
!!! note "Замечание:"
    ...
    Таблицы, предварительно определенные в свойствах проекта или формы, задаются с помощью свойств проектаСвойство блока: Текст замены [n]и свойств формыФорматированные свойства: Текст замены [n]. При помощи индекса [n] можно определить там до 100 таблиц для свойств проекта и до десяти таблиц для свойств формы.
...
Выберите из раскрывающегося списка язык, на котором выводится свойство. Для каждого свойства язык может устанавливаться отдельно.
  ...
  * <Язык>: доступно только для форматов для свойств блока. Если для свойства выбран этот параметр, (например, "en_US (английский (США)")), переведенный текст отображается на этом языке. В этом списке представлены все языки, указанные в настройках перевода как языки перевода.
...
Настройки для языков (исходный язык, языки отображения и т. д.) выполняются в специфических для проекта настройках перевода (Файл > Настройки > Проекты > "Имя проекта" > Перевод > Общее).



!!! tip "Совет:"

    Используйте такой параметр, как "Язык отображения 1", "Язык отображения 2" и т. д., чтобытексты свойствблока и тексты-заполнители в основных данных (макросы, формы,рамкии т. д.) всегда отображались на текущем языке отображения, даже если он изменяется в настройках перевода. При выборе определенного языка отображения (например, "ru_RU") тексты свойств в основных данных всегда отображаются на этом языке, даже если язык отображения изменяется в настройках перевода.
...
[Диалоговое окно Тексты замены](eservicesgui_d_ersetzungstexte.htm)

[Диалоговое окно "Настройки: Общее" (проект, перевод) / Вкладка "Общее"](translategui_r_einstellungprojekt.htm)
