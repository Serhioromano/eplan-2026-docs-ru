...
В Eplan можно импортировать файлы конфигурации ПЛК, сгенерированные или обработанные во внешних программах конфигурации ПЛК. Эти файлы конфигурации содержат структуры каркаса и структуры сети (при наличии).

Формат файла импорта автоматически распознается для многих распространенных программ конфигурации ПЛК при использовании функции [Импортировать данные ПЛК](plcgui_d_importbuskonfig.htm). В зависимости от этого формата данные из импортируемого файла присваиваются свойствам Eplan. При импорте Eplan добавляет данные функциональных элементов как неиспользованные входы / выходы из данных изделий. Качественная база данных изделий облегчает дальнейшее использование импортированных данных.
...
Изделие подходит устройству, если в файле импорта и базе данных изделий совпадает свойство Обозначение типа ПЛК или Описание устройства: Имя файла. Свойство Обозначение типа ПЛК сравнивается, если не указан [файл описания устройства](plcgui_k_austauschbuskonfig.htm#I_GSDDatei). Или просто обозначение типа ПЛК соответствует номеру для заказа. После импорта данные изделия автоматически синхронизируются с базой данных изделий. При этом изделия, содержащиеся в базе данных изделий, но пока отсутствующие в проекте, загружаются в проект.

  * Если указан файл описания устройства, то для форматов AutomationML AR APC дополнительно сопоставляется свойство Описание устройства: Индекс в файле. Индекс позволяет выбрать устройство внутри такого файла независимо от языка. Для всех остальных форматов используется свойство Описание объекта.
...
!!! example "Пример:"
    ...
    Проект шинного интерфейса содержит две карты типа "IB IL 24 DI 16-PAC". Это устройство также содержится в базе данных изделий ("PXC.2861250"). В базе данных изделий генерируется второй вариант, и оба варианта содержат разные номера версий (свойствоВерсиявкладкиСвойства). Данные номера версий также устанавливаются на картах в проекте шинного интерфейса. Данные будут экспортированы, а затем импортированы в пустой проект. После импорта номера версий карт в пустом проекте отличаются друг от друга, и обе карты ссылаются на разные варианты одного и того же изделия (вкладкаИзделиев диалоговом окне свойств).
...
При импорте в формате AutomationML AR APC — если отсутствуют изделия с точно таким же обозначением типа ПЛК — также присваиваются изделия, обозначения типа ПЛК в которых отличаются только разным количеством пробелов.
...
[Обмен данными файлов конфигурации ПЛК](plcgui_k_exportbuskonfig.htm)
