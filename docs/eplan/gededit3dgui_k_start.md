...
!!! tip "Совет:"
    ...
    Чтобы перейти от 3D-размещения изделия или функции из пространства листа или из схемы соединений к обзору модели с перекрестными ссылками, можно использовать команды **Перекрестная функция** > **Список**. Они доступны, например, в виде пунктов всплывающего меню в открытом пространстве листа или в графическом редакторе. После выполнения этого действия обзоры модели с перекрестными ссылками — при их наличии — будут перечислены в диалоговом окне **Результаты поиска** в списке **Перейти к**. Если затем из списка **Перейти к** с помощью пункта всплывающего меню **Перейти к (графика)** переключиться на обзор модели, то в обзоре модели будет выделено представление функции с перекрестной ссылкой. Если сама функция в обзоре модели не видна (например, в отображении отверстий) или если для обзора модели выбран стиль с "оттенением", то это будет невозможно и тогда выделяется весь обзор модели.
...
!!! example "Пример:"
    ...
    (1): Вид на косую поверхностьбезактивированногосвойства**Обзор модели: Занять точку наблюдения за монтажной поверхностью**и результат в обзоре модели(2): Вид на косую поверхностьсактивированным свойством**Обзор модели: Занять точку наблюдения за монтажной поверхностью**и результат в обзоре модели
