...
!!! tip "Совет:"
    ...
    Чтобы перейти от 3D-размещенияизделияилифункциииз пространства листа или изсхемы соединенийк обзору модели с перекрестными ссылками, можно использовать командыПерекрестная функция>Список. Они доступны, например, в виде пунктов всплывающего меню в открытом пространстве листа или в графическом редакторе. После выполнения этогодействияобзоры модели с перекрестными ссылками — при их наличии — будут перечислены в диалоговом окнеРезультаты поискав спискеПерейти к. Если затем из спискаПерейти кс помощью пункта всплывающего менюПерейти к (графика)переключиться на обзор модели, то в обзоре модели будет выделено представление функции с перекрестной ссылкой. Если сама функция в обзоре модели не видна (например, в отображении отверстий) или если для обзора модели выбран стиль с "оттенением", то это будет невозможно и тогда выделяется весь обзор модели.
...
!!! example "Пример:"
    ...
    (1): Вид на косую поверхностьбезактивированногосвойстваОбзор модели: Занять точку наблюдения за монтажной поверхностьюи результат в обзоре модели(2): Вид на косую поверхностьсактивированным свойствомОбзор модели: Занять точку наблюдения за монтажной поверхностьюи результат в обзоре модели
