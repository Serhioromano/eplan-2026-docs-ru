...
!!! tip "Совет:"
    ...
    Чтобы перейти от 3D-размещенияизделияилифункциииз пространства листа или изсхемы соединенийк обзору модели с перекрестными ссылками, можно использовать команды Перекрестная функция>Список. Они доступны, например, в виде пунктов всплывающего меню в открытом пространстве листа или в графическом редакторе. После выполнения этогодействияобзоры модели с перекрестными ссылками — при их наличии — будут перечислены в диалоговом окне Результаты поискав списке Перейти к. Если затем из списка Перейти кс помощью пункта всплывающего меню Перейти к (графика)переключиться на обзор модели, то в обзоре модели будет выделено представление функции с перекрестной ссылкой. Если сама функция в обзоре модели не видна (например, в отображении отверстий) или если для обзора модели выбран стиль с "оттенением", то это будет невозможно и тогда выделяется весь обзор модели.
