...
[Вызов диалогового окна (Форматы для свойств блока):](javascript:void\(0\);)
...
[Вызов диалогового окна (определяемые пользователем свойства):](javascript:void\(0\);)
...
[Вызов диалогового окна (шаблон для автоматического выделения):](javascript:void\(0\);)
...
[Вызов диалогового окна (формат отображения для определений сегмента):](javascript:void\(0\);)
...
[Вызов диалогового окна (форматы блока для конфигурации представления структуры дерева):](javascript:void\(0\);)
...
---|---  
|  Свойство можно выбрать двойным щелчком по элементу формата.  
|  С помощью кнопки  (Следующая степень) можно перейти от исходного объекта к другим связанным с ним объектам и выбрать косвенные свойства.  
|  Можно выбрать свойство (двойным щелчком), а также перейти к другим объектам (с помощью кнопки  (Следующая степень)).  
|  Группа свойств  
...
Нажмите на символ , чтобы открыть группы свойств более низкого уровня. Если от объекта можно перейти к косвенным свойствам последующих объектов, над полем Навигация активна кнопка  (Следующая степень). Щелкните по ней, и снова отобразится первоначальная группировка свойств – теперь это (косвенные) свойства дополнительного объекта! Если выбирается группа форматов, то кнопка  (Переместить направо) активна. В дополнительном диалоговом окне выберите определенное свойство и установите для него форматирование.
...
[Функции](javascript:void\(0\);)

Форматы для свойств блока: для функций свойства сгруппированы следующим образом:

  * Функция: Прямые свойства текущей функции. Текущая функция может быть также связана с начальной функцией, которая при навигации находится с помощью  (Следующая степень).
  ...
  * Главная функция: Обеспечивает на вспомогательных функциях доступ к косвенным свойствам главной функции (при помощи  (Следующая степень)).
  * Главная функция вышестоящего устройства: В случае с вложенными устройствами обеспечивает доступ из подчиненного устройства к свойствам главной функции вышестоящего устройства (при помощи  (Следующая степень)). Если главная функция не была обнаружена, выводятся свойства первой вспомогательной функции. Чтобы достичь самого вышестоящего устройства у многократно вложенных устройств, необходим многократный переход через  (Следующая степень).
  ...
  * Вышестоящее 3D-размещение изделия: Предоставляет доступ к свойствам вышестоящего 3D-размещения изделия в навигаторе пространства листа (через  (Следующая степень)).
  * Функциональный текст зоны: Предоставляет доступ к соответствующему функциональному тексту зоны (через  (Следующая степень)).
  ...
  * Разделитель: Открывает [диалоговое окно Формат: Разделитель](eservicesgui_d_formattrennzeichen.htm), в котором задается форматирование для разделителя в группе форматов.
  * Комментарий: Открывает [диалоговое окно Формат: Комментарий](eservicesgui_d_formatkommentar.htm), в котором можно задать комментарий в пределах группы форматов и с помощью которого документируются настройки формата.
  * Вычисление: открывает [диалоговое окно Формат: Вычисление](eservicesgui_d_formatberechnung.htm), в котором можно составить расчетную формулу.



[Точки разрыва](javascript:void\(0\);)
...
Форматы для свойств блока: для точек разрыва свойства сгруппированы следующим образом:
  ...
  * Точка разрыва (обр. эквивалент): Обеспечивает на точках разрыва по типу звезда доступ к косвенным свойствам различных обратных эквивалентов (при помощи  (Следующая степень)).
  * Точка разрыва (источник): Обеспечивает доступ к свойствам источника точки разрыва (при помощи  (Следующая степень)).
  * Цель: Свойства целей, которые подсоединены к точке разрыва.
  * Соединение: Обеспечивает доступ к косвенным свойствам соединений, через которые закрываются цели на точках разрыва (при помощи  (Следующая степень)).
  * Страница: Свойства страницы, на которой находится текущая функция.
  * Проект: Свойства текущего проекта.
  * Разделитель: Открывает [диалоговое окно Формат: Разделитель](eservicesgui_d_formattrennzeichen.htm), в котором задается форматирование для разделителя в группе форматов.
  * Комментарий: Открывает [диалоговое окно Формат: Комментарий](eservicesgui_d_formatkommentar.htm), в котором можно задать комментарий в пределах группы форматов и с помощью которого документируются настройки формата.
...
[Определения потенциала](javascript:void\(0\);)
...
[Определения трубопроводов](javascript:void\(0\);)
...
Форматы для свойств блока: свойства для точек определения трубопровода сгруппированы следующим образом:
  ...
  * Данные сегмента: предоставляет доступ к свойствам вышестоящих сегментов, таким как объекты планирования трубопроводов (с помощью элемента  (Следующая степень)).
...
[Автоматические тексты описания](javascript:void\(0\);)
...
Форматы для свойств блока / формат отображения навигатора: для сегментов в предварительном планировании свойства сгруппированы следующим образом:
  ...
  * Проект: Свойства текущего проекта.
  * Размещение: В размещенных сегментах предоставляет доступ к свойствам самого размещения (имя страницы, номер столбца и т. д.) и к свойствам страницы через  (Следующая степень).
...
[Объекты в представлении структуры дерева](javascript:void\(0\);)
...
!!! example "Пример:"
    ...
    Вы выделили функцию на схеме соединений и выбрали элемент формата "ОУ (видимое)".Видимое ОУвыделенной функции отобразится в столбцеПредв. просмотр.
...
!!! example "Пример:"
    ...
    Вы выделили клемму и хотите вывести ОУ первой цели, подсоединенной к первому подключению клеммы.Для этого в полеДоступные элементы форматавыберите записиЦель через вывод устройства и номер цели > Вывод устройства 1 > Цель 1и щелкните по кнопке. В полеДоступные элементы форматавы можете теперь выбрать свойства клеммной цели. Выберите записиСвойства функции > Свойства устройства, и щелкните по. Откроется диалоговое окноФормат: Свойство блока; выберите в нем свойствоОУ (полное)и щелкните[OK].Если вы теперь выделяете запись в полеВыбранные элементы формата, в полеНавигацияпоказывается следующее:Клемма, 2 выводов устройстваЦель через вывод устройства и номером цели, Вывод устройства 1, Цель 1Свойства устройства
...
Кнопка |  Значение  
---|---  
(Следующая степень) |  Обеспечивает переход от исходного объекта к другим, связанным с ним объектам, а также выбор косвенных свойств.  
(Предыдущая ступень) |  Позволяет переходить от косвенно запрошенного объекта обратно к исходному объекту.  
...
[Диалоговое окно Конфигурировать представление (дерево)](modaldialogsdb_d_baumkonfiguration.htm)
