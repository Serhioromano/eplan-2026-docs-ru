...
![](images/arrow.png) Через каждый кабельный канал прокладывается автоматический сегмент маршрутизации.

![](images/arrow.png) Сегменты маршрутизации кабельных каналов, размещенных рядом друг с другом, соединяются. Открытым концам каналов присваивается поперечный сегмент маршрутизации, чтобы в них можно было поместить соединения.

![](images/arrow.png) Области маршрутизации размещаются без сегментов маршрутизации целиком в качестве части сети соединенных сегментов.

![](images/arrow.png) Информация обо всех сегментах маршрутизации, найденных в пространстве листа (геометрическая длина, направление, предшествующий сегмент, следующий сегмент), собирается и объединяется в сеть соединенных сегментов. Эта сеть служит информационной основой при расчете трассы маршрутизации.
