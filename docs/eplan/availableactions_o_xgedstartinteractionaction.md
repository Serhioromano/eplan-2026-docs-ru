...
Эта функциональность предусмотрена только в определенных модулях расширения. [Информация / авторское право](eplan/license_k_start.md) Вы находитесь здесь:
...
С помощью этих взаимодействий есть возможность устанавливать настройки форматирования условных обозначений, текстов, графических элементов и точек определения соединений и потенциалов на точно заданные значения. При этом возможности настройки идентичны тем, которые также можно определить в интерфейсе пользователя в соответствующих диалоговых окнах свойств на вкладках Отображение, Формат и Графика соединения.

В Eplan можно назначить эти взаимодействия [определенной пользователем группе команд в качестве команд](eplan/userinterface_h_menuebandanpassen.md), например, в ленте. Далее перечисляются параметры вышеназванных взаимодействий.

!!! example "Пример:"

    Для определенных текстов следует устанавливать следующие значения:Размер шрифта: 10 мм,Цвет: зеленый,Угол: -90°. Кроме того, должен отображаться блок выравнивания шириной 45 мм и высотой 20 мм. Один из возможных вариантов ввода в командной строке диалоговогоПереименоватьможет выглядеть следующим образом:XGedStartInteractionAction/Name:XGedIaFormatText /height:10mm /color:3 /justification:? /angle:-90 /setframeactive:1 /showframe:1 /framewidth:45mm /frameheight:20mmЕсли свойство не изменится, то просто оставьте соответствующий вопросительный знак "?" в командной строке. Например, свойствоВыравнивание(параметр:/justification:?). Ненужные параметры, например/bold:?, можно убрать из командной строки.
...
Но в диалоговом окне [Выбор цвета](eplan/mf_d_farbauswahl.md) можно устанавливать и другие значения. Возможны значения от 0 до 256. Значения для цветов отображаются во всплывающей подсказке Выбора цветов.  
...
/font:? |  Вид шрифта |  Значения для видов шрифтов 1–10 задаются в фирменных настройках: Файл > Настройки > Фирма > Графическая обработка > Виды шрифтов. Доступны следующие значения:
...
!!! example "Пример:"
    ...
    В случае с определенными графическими элементами толщина линии должна быть установлена на 5,5 мм, а графика - окрашена в цвет циан. Графические элементы должны быть видимы в любом случае. Ввод в командную строку может выглядеть следующим образом:XGedStartInteractionAction /Name:XGedIaFormatGraphic /width:5,5mm /color:4 /visible:1

 

!!! example "Пример:"

    При определенных условных обозначениях на страницесхемы соединенийфункциональный текст и другие размещенныетексты свойствнастраиваются на французский язык. Ввод в командную строку может выглядеть следующим образом:XGedStartInteractionAction /Name:XGedIaFormatSymbol /language:fr_FR
...
/showtextbox:? |  Начертить рамку текста |  Определяет, нужно ли применять рамку текста (является опцией). Доступны следующие значения:

* 0 = Не показывать рамку текста
* 1 = отображать рамку текста как прямоугольник
* 2 = отображать рамку текста как эллипс
* 3 = Из слоя.
* 4 = отображать рамку текста как овал
...
!!! example "Пример:"
    ...
    Для определенных текстов следует устанавливать следующие значения:Размер шрифта: 10 мм,Цвет: зеленый,Угол: -90°. Дополнительно должен отображаться блок выравнивания со следующими значениями:Ширина: 45 мм,Высота: 20 мм. Ввод в командную строку может выглядеть следующим образом:XGedStartInteractionAction /Name:XGedIaFormatText /height:10mm /color:3 /angle:-90 /setframeactive:1 /showframe:1 /framewidth:45mm /frameheight:20mm
...
* 11 = функциональный
* 12 = предварительное планирование
* 13= обзор функций (Fluid-Техника).
