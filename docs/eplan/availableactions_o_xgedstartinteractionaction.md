...
Эта функциональность предусмотрена только в определенных модулях расширения. [Информация / авторское право](license_k_start.htm) Вы находитесь здесь:
...
В Eplan можно назначить эти взаимодействия [определенной пользователем группе команд в качестве команд](userinterface_h_menuebandanpassen.htm), например, в ленте. Далее перечисляются параметры вышеназванных взаимодействий.
...
Но в диалоговом окне [Выбор цвета](mf_d_farbauswahl.htm) можно устанавливать и другие значения. Возможны значения от 0 до 256. Значения для цветов отображаются во всплывающей подсказке Выбора цветов.  
