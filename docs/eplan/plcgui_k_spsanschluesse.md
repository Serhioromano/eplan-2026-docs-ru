...
Описание вывода устройства для каждого канала может быть настроено только один раз, но для каждой карты возможно многократное появление. Питания модулей могут иметь также одинаковые описания выводов устройства.
...
Для всех систем шин. В системе шин на базе Ethernet выводы устройства должны иметь одинаковое имя интерфейса шины. Имя интерфейса шины не должно быть пустым. Введите данные только на главном выводе шины. С помощью контрольного прогона [004040](messages_p_004040.htm) можно найти противоречивые данные.
...
Выводы шин с пустым ОУ игнорируются при поиске сопряженных выводов шин. Это ускоряет определение структур сети, когда есть много выводов шин с пустым ОУ.



!!! note "Замечание:"

    При обмене данными ПЛК вформате AutomationML AR APCвыводы шинывнутри одной и той же системы шин и одной и той же физической сети (свойствоФизическая сеть: Имя) считаются связанными, даже если они имеют разные ОУ. Однако это не видно при отслеживании сети. Контрольный прогон004104сообщает о таких соединениях шин как несвязанных между собой. Это сообщение можно проигнорировать, если вы намеренно спроектировали структуру сети таким образом из-за физических условий.
