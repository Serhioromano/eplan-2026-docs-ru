...
С помощью вида сегментации вы устанавливаете, как следует переводить предназначенные для перевода тексты: по словам, по предложениям или как общую запись. От вида сегментации также зависит, когда знак в конце предложения интерпретируется как конец предложения. Вид сегментации устанавливается в [специфических для проекта настройках перевода](translategui_r_einstellungprojekt.htm#NurGanzeSaetzeUebersetzen) и [специфических для пользователя настройках перевода](modaldialogs_r_einstellungprojekt.htm#Segment) в поле Сегмент.
