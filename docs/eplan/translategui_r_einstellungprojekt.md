...
## Диалоговое окно "Настройки: Общее" (проект, перевод) / Вкладка "Общее"

[Вызов диалогового окна:](javascript:void\(0\);)

  * Вы открыли проект. Файл > Настройки > Проекты > "Имя проекта" > Перевод > Общее.
  * Вкладка Инструменты > группа команд Перевод > раскрывающаяся кнопка Перевести > Настройки. Выберите вкладку Общее.



В этом диалоговом окне или на этой вкладке задаются настройки перевода и языки, на которые необходимо перевести тексты текущего проекта. Настройки перевода и отображения относятся к конкретному проекту. Это позволяет выполнять перевод проектов с разными настройками на одной машине.
...
!!! example "Пример:"
    ...
    записьCables w. Terminals. Circ.-Breaker and more.будет выведена в списке отсутствующих слов следующими восемью сегментами перевода:1-й сегмент перевода:Cables2. сегмент перевода:w3. сегмент перевода:Terminals4-й сегмент перевода:-Breaker5-й сегмент перевода:.6-й сегмент перевода:Circ7-й сегмент перевода:more8-й сегмент перевода:and
...
!!! example "Пример:"
    ...
    записьCables w. Terminals. Circ.-Breaker and more.будет выведена в списке отсутствующих слов следующими тремя сегментами перевода:1-й сегмент перевода:Cables w2. сегмент перевода:Terminals3. сегмент перевода:Circ.-Breaker and more
...
!!! example "Пример:"
    ...
    В полеСегментвыбрана настройкаСлово, и на страницесхемы соединенийв виде текста введена следующая комбинация слов на русском языке:Not-AusПолеДополнительные разделители словне заполнено.На основе пустого словаря сгенерируйте список отсутствующих слов. Eplan выведет сегмент переводаNot-Aus.Теперь введите в полеДополнительные разделители словсимвол "-" (дефис). Необходимо провести повторную генерацию списка отсутствующих слов. Теперь Eplan выводит следующие 3 сегмента перевода в алфавитном порядке:-AusNotЕсли дополнительныйразделительслов был введен в словаре в список "Слова не переводить", то он не будет выведен в списке отсутствующих слов. Если дополнительный разделитель слов не внесен в этот список, то он будет выведен в списке отсутствующих слов.
...
Стандартно эта настройка действует для всех переводимых текстов проекта в графическом редакторе. Для отдельных текстов можно также установить только один язык отображения, выделив в диалоговом окне "Свойства" текста на вкладке Формат для свойства Формат > Язык значение Одноязычн. (перемен.).

Одноязычн. (перемен.):

Здесь можно выбрать язык отображения переведенных текстов проекта, свойству Язык которых вы присвоили значение Одноязычн. (перемен.). Это выполняется в диалоговом окне "Свойства" текста на вкладке Формат. Вы можете выбрать любой из языков перевода, которые вы указали в групповом поле Перевод.
...
[Определить языки перевода и языки отображения](translategui_h_sprachenwaehlen.htm)
