...
[Вызов диалогового окна:](javascript:void\(0\);)
  ...
  * Вкладка Устройства > группа команд Устройства > Навигатор
  * Вы открыли редактор, например графический редактор, редактор рамок и т. д. Вкладка Вид > группа команд Навигаторы > раскрывающаяся кнопка Открыть > Устройства
...
[Пиктограммы](userinterface_k_iconsnavigatoren.htm) указывают на статус и вид представления функции.
...
Если в дереве навигатора устройств выделить одну функцию, то все зависимые функции выделятся полужирным шрифтом. В зависимости от режима обработки (вкладка Обработать > группа команд Параметры > Общие) здесь есть две возможности.

Режим-Свойства (общие) |  Выделение  
---|---  
Отключено |  Выделяются все функции, которые применяют ОУ выделенной функции.  
Активировано |  Выделяются все функции, относящиеся к устройству.  
...
!!! note "Замечание:"
    ...
    Обратите внимание, что при удалении размещенных устройств могут измениться также имеющиеся в проектесоединения. Соединения могут быть удалены, или могут возникнуть новые.
...
!!! note "Замечание:"
    ...
    С помощью пункта всплывающего менюНовое устройствотакже можно выбирать изделия из главной группы продуктов "Механика" и создавать их в качестве устройства. При этом создается механическое устройство с неразмещенными механическими функциями. С помощью пунктов менюСоздатьиНовые функцииневозможногенерироватьмеханические функции, т. к. создание механических функций без изделий нецелесообразно.
...
[Присвоить данные функции условному обозначению](navigatorgui_h_funktionzuweisen.htm)
