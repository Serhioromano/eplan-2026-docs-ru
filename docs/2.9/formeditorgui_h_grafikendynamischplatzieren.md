...
Чтобы [создать](Glossary_o_erstellen.md) верхний колонтитул, область данных и нижний колонтитул в пределах страницы, используется концепция динамических форм. Т.к. позиции этих областей динамичны и хорошо выделяемы в графике таблицы, то строка в динамичной форме, созданная в отчете, способна сама чертить рамку.

Обычно верхний колонтитул, область данных и нижний колонтитул используется только для форм, из которых создаются относящиеся к [функции](Glossary_o_funktionen.md) [отчеты](Glossary_o_auswertungen.md), к примеру [спецификации](Glossary_o_stuecklisten.md) клеммников. Для обзоров, таких, как, например, спецификации изделий, верхний колонтитул несущественен.

!!! note "Замечание:"

    Динамические [формы](Glossary_o_formulare.md) вы отличите по тому, что для [свойства](Glossary_o_eigenschaften.md) формы Работа с формой была выбрана запись "Динамич." из раскрывающегося списка.
...
Эта информация отмечается в редакторе формы как рамка строки и с каждой созданной строкой записывается в страницу отчетов. Кроме того, возможно также определять текстовые и графические [объекты](Glossary_o_objekte.md) как верхний колонтитул, элементы данных и нижний колонтитул, а не только тексты-заполнители.
...
* Как и все другие формы, во время обработки могут быть автоматически проверены также и [динамические формы](Glossary_o_dynamischeformulare.md) (Сервисные программы > Проверить форму), например, правильно ли применяются области определения.

### Присваиваются тексты заполнителя, находящиеся за пределом области

Кроме того, для динамических форм также возможно [присоединить](Glossary_o_andocken.md) к областям тексты заполнителя, которые расположены вне областей.
