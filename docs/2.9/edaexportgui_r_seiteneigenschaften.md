# Вкладка Свойства страницы

Вы открыли проект.

* Проект > Опубликовать. Щелкните в поле Схема диалогового окна Опубликовать на ++...++. Выберите вкладку Свойства страницы.
* Параметры > Настройки > Пользователь > Интерфейсы > Опубликовать. Выберите вкладку Свойства страницы.

На вкладке Свойства страницы можно просмотреть [свойства](Glossary_o_eigenschaften.md) страниц экспортируемого проекта, но нельзя их изменить.

Обзор основных элементов диалогового окна:

### Имя свойства:

В этом столбце отображаются свойства страниц выделенного проекта.

### Панель инструментов:

### Всплывающее меню:

Всплывающее меню дает доступ, в зависимости от типа поля (например, дата, целое число, многоязычный), к пунктам меню, при помощи которых вы можете по необходимости, например, влиять на представление таблиц или обрабатывать значения в полях. Обзор пунктов этого всплывающего меню вы можете найти в разделе [Пункты всплывающего меню](userinterface_m_kontextmenu.md).

**См. также:**

* [Диалоговое окно Опубликовать](edaexportgui_d_veroeffentlichen.md)
* [Вкладка Экспорт](edaexportgui_r_export.md)
* [Вкладка Свойства проекта](edaexportgui_r_eigenschaften.md)
* [Вкладка Свойства пространства листа](edaexportgui_r_bauraumeigenschaften.md)
* [Вкладка Свойства функции](edaexportgui_r_funktionseigenschaften.md)
* [Вкладка Свойства точек разрыва](edaexportgui_r_abrstleigenschaften.md)
С вашей помощью мы можем улучшить работу системы. Мы документируем ваши действия в Google Analytics, чтобы постоянно совершенствовать справочную систему ([Дополнительная информация и возможности подачи возражений](helpsystem_hinweise_optout.md)).

Скрыть сообщение
