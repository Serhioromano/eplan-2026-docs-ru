...
В соответствии с классом атрибуту [Start], а также [функции](Glossary_o_funktionen.md) в [сценарии](Glossary_o_scripte.md) в первую очередь нужно задать набор параметров операции, а затем выполняется операция CommandLineInterpreter().Execute().
