...
* Данные проекта > Кабели > Навигатор. Выделите несколько значений, содержащих [кабели](Glossary_o_kabel.md), и выберите во всплывающем меню Автоматический выбор кабеля. Щелкните в диалоговом окне Автоматический выбор кабеля рядом с полем Настройки по ++...++.

В этом диалоговом окне задаются [настройки](Glossary_o_einstellungen.md) для автоматического выбора кабеля; настройки сохраняются как схема.
...
Кнопка |  Значение
---|---
![](images/all_new_as.png){: .ui-icon } (Создать) |  Открывает диалоговое окно Выбор изделия. Выбранное [изделие](Glossary_o_artikel.md) добавляется в список предварительного выбора.
...
В таблице для каждого типа кабеля можно дополнять или изменять сохраненные из [базы данных изделий](Glossary_o_artikeldatenbanken.md) заданные значения. При автоматическом выборе кабеля EPLAN производит поиск подходящего кабеля в базе данных изделий. При этом сравниваются, например, подаваемое на кабель напряжение, напряжение на кабельных соединениях (определяется через [потенциалы](Glossary_o_potenziale.md)), имеющееся количество соединений и число резервных кабельных соединений.
...
При расчете числа кабельных соединений учитываются РЕ- / PEN-[жилы](Glossary_o_adern.md), а экранирование не учитывается.
