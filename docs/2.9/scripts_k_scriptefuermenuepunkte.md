...
С помощью сценария можно добавить к меню Сервисные программы один или несколько пунктов меню. Для этого в соответствующем [сценарии](Glossary_o_scripte.md) используется атрибут [DeclareMenu], через этот атрибут вызывается функция MenuFunction(). При загрузке сценария создается пункт меню, определяемый через функцию AddMenuItem.
