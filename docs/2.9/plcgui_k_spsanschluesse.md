...
Вывод [устройства](Glossary_o_betriebsmittel.md) ПЛК, В/В, относится к карте ПЛК или каналу на карте. Вывод устройства всегда имеет обозначение вывода устройства и часто также описание вывода устройства. Вывод устройства ПЛК, В/В, однозначно идентифицируется с помощью ОУ его блока ПЛК, его [обозначения](Glossary_o_bezeichnungen.md) штекера и его обозначения вывода устройства.

***Обозначение вывода устройства*** обычно может встречаться в каждой карте только один раз, но несколько раз в одном ПЛК. Если [выводы устройства](Glossary_o_anschluesse.md) ПЛК дополнительно различаются по обозначению штекера, обозначения выводов устройства могут встречаться несколько раз внутри одной карты.
...
* ***Вид сигнала***: Для настраиваемых выводов устройства ПЛК это свойство определяет, о каком выводе устройства идет речь. Кроме того, вид сигнала анализируется при представлении перекрестной ссылки на странице обзора. При помощи определенной [настройки](Glossary_o_einstellungen.md) [перекрестные ссылки](Glossary_o_querverweise.md) можно отобразить не только на выводах устройства, используемых в качестве входов и выходов, но и на точках подвода питания.
...
В области устройств ПЛК все чаще появляются устройства с настраиваемыми выводами устройства ПЛК: выводы устройства ПЛК могут быть либо входами, либо выходами. Для отображения карт ПЛК с такими выводами устройства в новой версии EPLAN служит определение [функции](Glossary_o_funktionen.md) "Вывод устройства ПЛК, многофункциональный".
...
Чтобы отобразить выводы шин и выводы сети в EPLAN, используются [определения функций](Glossary_o_funktionsdefinitionen.md) из категории "Кабельный вывод сети / шины". Такие выводы устройства по умолчанию ***соединяют сети*** и ***передают сигнал***. С их помощью устройства за пределами ПЛК или системы шины можно объединить в [сеть](Glossary_o_netze.md).
...
При импорте файлов [конфигурации](Glossary_o_konfigurationen.md) ПЛК выводы устройства из файла импорта также присваиваются выводам устройства в проекте, только если системы шин идентичны или совместимы. Если системы шин несовместимы, то при импорте генерируются новые дополнительные выводы устройства ПЛК.
...
Для всех систем шин. В системе шин на базе Ethernet выводы устройства должны иметь одинаковое ***имя интерфейса шины***. Введите данные только на главном выводе шины. С помощью контрольного прогона [004040](messages_p_004040.md) можно найти противоречивые данные.
...
!!! note "Замечание:"
    ...
    При обмене данными ПЛК в ***формате AutomationML*** [выводы шины](Glossary_o_sps_busanschluesse.md) внутри одной и той же системы шин и одной и той же физической сети (свойство Физическая сеть: Имя) считаются связанными, даже если они имеют разные ОУ. Однако это не видно при отслеживании сети. Контрольный прогон [004104](messages_p_004104.md) сообщает о таких соединениях шин как несвязанных между собой. Это сообщение можно проигнорировать, если вы намеренно спроектировали структуру сети таким образом из-за физических условий.
