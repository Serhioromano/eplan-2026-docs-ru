...
Перемычки можно [создать](Glossary_o_erstellen.md) различными способами. Перемычку в точке определения [соединения](Glossary_o_verbindungen.md) Вы создаете вручную, задавая там тип перемычки соединения. Так, например, Вы можете определить вставную или проволочную перемычку.
...
* Вы открыли проект.
* На странице [схемы соединений](Glossary_o_anschlussbilder.md) проекта определен клеммник и вставлено несколько находящихся рядом друг с другом клемм, которые имеют ОУ клеммника и носят различные [обозначения](Glossary_o_bezeichnungen.md).

Ниже различные возможности по созданию перемычек разъясняются на примере. На странице [схемы](Glossary_o_schemata.md) соединений с помощью двух углов и тройника между собой связаны три клеммы.
...
!!! tip "Совет:"
    ...
    В диалоговом окне Обработать клеммник с помощью специальных кнопок можно [генерировать](Glossary_o_erzeugen.md) или удалять [вручную созданные мостовые перемычки](terminalgui_k_verwendungbruecken.md). Мостовые перемычки можно отдельно генерировать для внутренних и внешних выводов мостовых перемычек.

### Соединить клеммы посредством переключаемых перемычек

Переключаемая перемычка соединяет клемму со ***следующей*** клеммой с возможностью переключения. Используя [свойства](Glossary_o_eigenschaften.md) Переключаемая перемычка (внешняя) и Переключаемая перемычка (внутренняя), можно отдельно определить переключаемые перемычки для внутренней и внешней стороны клеммы. На следующем примере показано, как создаются внешние переключаемые перемычки.
