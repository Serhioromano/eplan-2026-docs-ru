...
Благодаря использованию шаблонов проекта порядки свойств для 3D-[размещения изделий](Glossary_o_artikelplatzierungen.md) уже доступны в новом проекте. Для условных обозначений порядки свойств отображаются, только если здесь в диалоговом окне или на самом условном обозначении сохранен определенный пользователем порядок свойств под именем.

Пока определенные пользователем порядки свойств 3D-размещения изделий в пространстве листа присвоены определению [функции](Glossary_o_funktionen.md), порядки свойств для условного [обозначения](Glossary_o_bezeichnungen.md) сохраняются для варианта символа.
...
Пункт меню |  Значение
---|---
С ориентацией на библиотеку (только дерево) |  Доступно только в порядках свойств для условных обозначений. Отображает [библиотеки символов](Glossary_o_symbolbibliotheken.md) в самом верхнем узле дерева в папке "Условное обозначение". Ниже находятся [определения функций](Glossary_o_funktionsdefinitionen.md).
С ориентацией на функцию (только дерево) |  Доступно только в порядках свойств для условных обозначений. Отображает функциональную категорию (например, "Катушки и контакты", "Сигнальные [устройства](Glossary_o_betriebsmittel.md)" и т. д.) в самом верхнем узле дерева в папке "Условное обозначение".
...
Вставить в [список результатов](Glossary_o_ergebnislisten.md) поиска |  Проверяет использование выделенных порядков свойств в проекте и вносит найденные [объекты](Glossary_o_objekte.md) в список результатов поиска. Благодаря этому с помощью пункта всплывающего меню Перейти к (графика) можно найти используемые порядки свойств объектов, например [условные обозначения](Glossary_o_schaltzeichen.md), размещения изделий и т. д., в графике.
...
Если в дереве выбран более высокий уровень иерархии, выделенный контур используется как [фильтр](Glossary_o_filter.md) для диалогового окна Определения функций или Выбор символа, которое открывается далее. Затем в диалоговом окне Новый порядок свойств введите однозначное идентифицирующее имя порядка свойств.
...
* При ***одинаковой конфигурации*** (одинаковые [свойства](Glossary_o_eigenschaften.md), последовательность свойств и настроек присоединения) порядки свойств можно непосредственно обрабатывать на вкладках Трехмерные функции, Условное обозначение и т. д.
...
После совместной обработки выбранные размещения свойств и измененные [настройки](Glossary_o_einstellungen.md) отображения доступны на всех выбранных порядках свойств.
