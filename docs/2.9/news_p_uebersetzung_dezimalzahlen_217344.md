...
Для перевода текстов, которые отличаются только числовыми значениями, в словарь можно заносить [ключевые слова](Glossary_o_stichworte.md) с заполнителями (напр. Voltage %0V). При этом заполнитель %0 ранее использовался для любой строки цифровых символов (например, 12 или 240 и т. д.). Теперь такой заполнитель может также использоваться для чисел, которые содержат, например, запятую, точку, косую черту или [предшествующий знак](Glossary_o_vorzeichen.md) (например, 12; 2,5; +-2.5; 3/4 и т. д.).

![](images/benefit.png)Эффект:

При использовании новой [настройки](Glossary_o_einstellungen.md) для распознавания десятичных знаков в словаре будет значительно меньше ключевых слов для однотипных текстов с различными комбинациями чисел и символов.
...
[![](images/translategui_newusersetting_ls_thumb_0_60.png)](../Pictures/Gui/Lang/translategui_newusersetting_ls.png)
