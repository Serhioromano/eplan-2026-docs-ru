...
Функции ПЛК, перечисленные в диалоговом окне Генерировать схему соединений ПЛК, используются при генерации [схемы соединений](Glossary_o_anschlussbilder.md), т. е. присваиваются соответствующим функциям в макросах.

Функции в макросах являются соответствующими при следующих условиях:

* При работе с ***Выводами устройства ПЛК*** должны соответствовать друг другу категория определения [функции](Glossary_o_funktionen.md) и обозначение выводов [устройства](Glossary_o_betriebsmittel.md).
...
* При работе с ***Блоками ПЛК*** требуется соответствие [определения функций](Glossary_o_funktionsdefinitionen.md).

Макросы могут содержать функции, которые ***не*** будут использованы, например избыточные функции ПЛК или функции, не являющиеся блоками или выводами устройства ПЛК. Таким образом, например, [макросы](Glossary_o_makros.md) проводки функций ПЛК не содержат.

Эти функции содержат данные из макроса, или, если макрос содержит [объекты-заполнители](Glossary_o_platzhalterobjekte.md), из наборов значений, присвоенных переменным. Переменные при этом не присвоены для присвоения значений функциям ПЛК; свои значения функции получают при использовании.
