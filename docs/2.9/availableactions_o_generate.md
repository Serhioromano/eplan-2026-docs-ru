...
CONNECTIONS: Генерация [соединения](Glossary_o_verbindungen.md)
...
CREATIONSCHEME |  Имя [схемы](Glossary_o_schemata.md), для генерации кабеля (необязательно).
...
Значение по умолчанию: последняя схема.
Действительно только для генерации кабеля.
REGENERATECONNS |  Указывает, следует ли сначала [генерировать](Glossary_o_erzeugen.md) новые соединения (необязательно, 0 = Нет, 1 = Да).
...
PAGEFILTERNAME |  Имя фильтра страницы. На страницах обновляются только те соединения, которые прошли через [фильтр](Glossary_o_filter.md) с именем PAGEFILTERNAME. Этот параметр оказывает влияние, только когда параметр TYPE имеет следующее значение: CONNECTIONS.
