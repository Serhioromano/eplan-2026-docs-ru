...
* Вы открыли ***Проект макросов*** и вставили в этом проекте [объекты](Glossary_o_objekte.md) (например, страницы с фрагментами [схемы соединений](Glossary_o_anschlussbilder.md) в рамках макроса) для генерируемых макросов. Данные проекта > Макросы > Навигатор.
* Вы открыли ***проект схемы соединений***. Открыта страница в графическом редакторе. Вы вставили [макросы](Glossary_o_makros.md) окна / страницы.

В этом диалоговом окне отображаются макросы, которыми вы управляете в проектах макросов или которые вы вставили в [проекты](Glossary_o_projekte.md) схем соединений.

* В ***проекте макросов*** для подготовленного макроса отображаются все [виды представления](Glossary_o_darstellungsarten.md) и [варианты](Glossary_o_varianten.md), которые затем сохраняются в файле макроса при генерации. Это относится ко всем типам макросов.
...
!!! note "Замечание:"
    ...
    Чтобы в проекте [схемы](Glossary_o_schemata.md) соединений вставленные макросы окна и символа можно было отображать и обновлять в навигаторе макросов, необходимо также вставлять [рамки макросов](Glossary_o_makrokaesten.md). Для этого активируйте настройку проекта [Вставить также рамки макросов](gedviewer_d_einstellungenprojektallgemein.md). В качестве альтернативы при подготовке макросов в проекте макросов можно отдельно указать для каждого макроса, следует ли вместе с ним вставлять рамку макроса. Настройка, выполненная на рамке макроса с помощью раскрывающегося списка [Вставить также рамку макроса](macrosgui_r_makrokasteneinstellungen.md), имеет приоритет перед настройкой проекта.

С помощью всплывающего меню можно обработать [свойства](Glossary_o_eigenschaften.md) макросов.
...
Под макросами отображаются их виды представления и варианты. При этом для различных типов макросов используются разные [пиктограммы](Glossary_o_icons.md), а для видов представления — те же пиктограммы, что и для типов страниц.
...
* Макросы окон и символов: Вид представления [рамки](Glossary_o_normblaetter.md) макроса (здесь, например, многополюсный)
...
!!! note "Замечание:"
    ...
    В списке навигатора можно обработать только один объект. По этой причине в представлении в виде списка навигатора макросов [объекты-заполнители](Glossary_o_platzhalterobjekte.md) недоступны.
...
Пункт меню |  Значение
---|---
Создать (только дерево) |  Этот пункт меню доступен только в проектах макросов. Позволяет создать страницу или [пространство листа](Glossary_o_bauraum.md) для нового макроса либо вставить рамку макроса (если открыта страница) через подменю. В зависимости от выбранного уровня иерархии (каталог, другой подготовленный макрос окна и т. д.) определенные данные копируются для нового подготовленного макроса.
...
Автоматически [генерировать](Glossary_o_erzeugen.md) макросы |  Этот пункт меню доступен только в проектах макросов. Автоматически генерирует макросы на основе текущего выбора (напр., отдельный подготовленный макрос).
...
Вставить в [список результатов](Glossary_o_ergebnislisten.md) поиска |  Позволяет вставить в список результатов поиска все выделенные объекты. Их можно снова отобразить позже через пункты меню Поиск > Отобразить результаты.
