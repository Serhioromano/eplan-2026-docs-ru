...
Теперь в диалоговом окне управления слоями можно искать все объекты, которым присвоен определенный слой в проекте или в его подразделе. Для этого на [панели инструментов](Glossary_o_symbolleisten.md) управления слоями доступна новая кнопка ![](images/all_search_as.png){: .ui-icon } (Поиск). Это позволяет, например, находить и корректировать слои, которые добавлены в проект посредством импорта [DXF](Glossary_o_dxf.md) / [DWG](Glossary_o_dwg.md) или используются как определенные пользователем слои в проекте.
