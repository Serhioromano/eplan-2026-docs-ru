...
В файле festo.cfg, который находится в каталоге ..\<Номер версии>\cfg, укажите импортируемые [свойства](Glossary_o_eigenschaften.md) изделия Festo. Здесь также выполняется присвоение изделия Festo к макросам или символам EPLAN. Дополнительно можно устанавливать собственные присвоения для свойств.

Файл [конфигурации](Glossary_o_konfigurationen.md) поделен на различные [разделы](Glossary_o_gewerke.md): В разделе "Festo-Settings" устанавливаются присвоения свойств. Таким образом Вы можете конфигурировать, как должны быть построены номера изделий. В разделе "Festo-MacroMapping" устанавливается присвоение макросов изделиям Festo.
...
* Строки комментария [обозначения](Glossary_o_bezeichnungen.md) при помощи "; " (; и при помощи пробелов).
...
* ***0 = Добавлять только новые записи данных:*** Это стандартные характеристики. Имеющиеся изделия не изменяются. Импортируются только новые, еще не [сохраненные изделия](Glossary_o_eingelagerteartikel.md).
...
!!! warning "Предупреждение:"
    ...
    Вы не должны изменять заданные свойства в разделе "Festo-Settings", т.к. они следят за тем, чтобы после импорта было создано [изделие](Glossary_o_artikel.md), функционирующее в EPLAN.
...
Чтобы внутри конфигурации временно [сохранить](Glossary_o_einlagern.md) значения (и не использовать свойства EPLAN не по назначению), в распоряжении имеются 10 переменных типа "Символьная строка". Переменные обозначаются с помощью предшествующего символа "$".
