...
При отображении точек [соединения](Glossary_o_verbindungen.md) символов соединения необходимо учитывать следующие особенности:

Тройники служат для разветвления автоматически сгенерированных линий соединения. Существуют тройники для четырех различных направлений. Для каждого направления имеется также четыре варианта. Эти [варианты](Glossary_o_varianten.md) определяют прохождение соединений.

Каждый тройник снабжен тремя выводами [устройства](Glossary_o_betriebsmittel.md). Точка без [обозначения](Glossary_o_bezeichnungen.md) указывает, откуда идет соединение. Точки, обозначенные "1" и "2" задают последовательность целей.
