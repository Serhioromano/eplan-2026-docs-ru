...
Области маршрутизации являются элементами [сети соединенных сегментов](Glossary_o_streckennetze.md). Соединения в областях маршрутизации не проходят по ходу сегмента маршрутизации: маршрутизация всегда выполняется посредством максимально короткого, прямого [соединения](Glossary_o_verbindungen.md). За счет этого можно, например, выполнить разводку на задней стороне монтажной платы.
...
* Функциональные элементы не должны находиться в [области маршрутизации](Glossary_o_verlegebereiche.md) или вторгаться в нее, так как это не предусмотрено маршрутизацией. За это условие отвечает контрольный прогон ([P026036](messages_p_026036.md)).
...
Графическое изменение размеров в областях маршрутизации возможно с помощью [функции](Glossary_o_funktionen.md) Обработка > Графика > Изменить длину. Графическое изменение возможно для одного из трех измерений.
...
4. После установки направления изменения растяните трехмерный объект в требуемом направлении. Можно использовать [следующие формы](Glossary_o_folgeformulare.md) ввода:
