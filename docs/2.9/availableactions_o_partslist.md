...
IMPORT: Импортировать [спецификации](Glossary_o_stuecklisten.md)
EXPORT: Экспортировать спецификации
IMPORTTOSYSTEM: Импорт в базу данных изделий
EXPORTFROMSYSTEM: Экспорт из [базы данных изделий](Glossary_o_artikeldatenbanken.md)
...
SQLFILTERPART  |  Опционально: SQL-[фильтр](Glossary_o_filter.md) для продвинутых пользователей.
...
CFGFILE  |  Опционально: каталог и имя файла [конфигурации](Glossary_o_konfigurationen.md).
Значение по умолчанию: используемый в данный момент файл конфигурации.
FIELDASSIGNMENTSCHEME  |  Опционально: имя [схемы](Glossary_o_schemata.md) присвоения полей для импорта изделий.
...
ADDITIONAL_LANGUAGE  |  Опциональн. Действительно, если TYPE = IMPORTTOSYSTEM. Если значение данного параметра равно 1, обновляются многоязычные [свойства](Glossary_o_eigenschaften.md) с другими параметрами языка. Если параметр не задан, значения многоязычных свойств заменяются содержимым файла.
