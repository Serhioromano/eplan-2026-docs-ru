...
Вставка изделий осуществляется при помощи пунктов меню Вставить > Принадлежности для маршрутизации. При размещении [принадлежности](Glossary_o_zubehoer.md) для маршрутизации доступны все стандартные [опции](Glossary_o_projektoptionen.md) пространства листа и диалоговое окно Опции размещения.
...
!!! note "Замечание:"
    ...
    Чтобы провести соединение через уже размещенное крепление для кабеля / шланга, необходимо [маршрутизировать](Glossary_o_verlegen.md) соединение с помощью [функции](Glossary_o_funktionen.md) Изменить маршрутизацию через крепление как целевой сегмент маршрутизации.
...
Для правильного расчета диаметра жгута на вкладке Данные соединения для изделия [соединения](Glossary_o_verbindungen.md) необходимо определить внешний диаметр (с указанием единицы измерения).
