# Макросы

Работа с макросами имеет следующие преимущества:

* Вы хотите повторно использовать фрагменты определенной [схемы соединений](Glossary_o_anschlussbilder.md), причем требуется использовать одну и ту же графику, но не всегда применять одни и те же данные и изделия.
* Вы хотите под определенными именами [сохранить](Glossary_o_einlagern.md) части [схемы](Glossary_o_schemata.md) соединений, чтобы позже снова использовать их.
* Вы хотите сохранить для макросов требуемые записи данных ([варианты](Glossary_o_varianten.md), т.е. таблицы значений для технических данных и элементов), чтобы уменьшить время, затрачиваемое на последующую обработку макросов после вставки. С этой целью для всех возможных данных, содержащихся в макросе, можно определить соответствующие [объекты-заполнители](Glossary_o_platzhalterobjekte.md).
* Существует возможность для создания проектов макросов и определения рамок макросов, с помощью последних можно, например, выполнить автоматическую генерацию макросов окон.

**См. также:**

* [Проекты макросов](macrosgui_k_makroprojekte.md)
* [Рамки макросов: принцип](macrosgui_k_makrokaesten.md)
* [Макросы: Защищенные группы](macrosgui_k_geschuetztegruppen.md)
* [Данные сгенерированного макроса ](macrosgui_k_daten.md)
* [Навигатор макросов](macrosgui_k_makronavigator.md)
* [Объекты-заполнители: Структура и принцип работы](macrosgui_k_platzhalteraufbauarbeitsweise.md)
* [Создать проекты макросов](macrosgui_h_makroprojekteerstellen.md)
* [Использовать рамки макросов](macrosgui_h_makrokasten.md)
* [Вставить макросы](macrosgui_h_makrosauswaehlen.md)
* [Создать макросы](macrosgui_h_makroserzeugen.md)
* [Генерировать макросы из файлов DXF / DWG](macrosgui_h_makrosausdxf.md)
С вашей помощью мы можем улучшить работу системы. Мы документируем ваши действия в Google Analytics, чтобы постоянно совершенствовать справочную систему ([Дополнительная информация и возможности подачи возражений](helpsystem_hinweise_optout.md)).

Скрыть сообщение
