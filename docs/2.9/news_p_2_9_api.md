...
С помощью EPLAN API теперь также можно автоматически масштабировать файлы [DXF](Glossary_o_dxf.md) во время импорта. С этой целью для метода DXFPage доступен новый класс DrawingLimits.

Новые возможности для точек монтажа

В классе Mate доступны два новых класса GridMate и LineMate, которые представляют определенные [точки монтажа](Glossary_o_montagepunkte.md).
...
В классе Insert3D теперь доступен новый вариант метода WindowMacro. Он позволяет [присоединить](Glossary_o_andocken.md) точку захвата к определенной точке монтажа другого объекта.

Выявить все объекты определенного типа

Класс DMObjectsFinder дополнен новым методом GetAll. Это позволяет выявлять все [объекты](Glossary_o_objekte.md) определенного типа.

Новый класс для создания определенных пользователем свойств в базе данных изделий

С помощью нового класса MDUserDefinedPropertyDefinition можно [создать](Glossary_o_erstellen.md) определенные пользователем [свойства](Glossary_o_eigenschaften.md) для [базы данных изделий](Glossary_o_artikeldatenbanken.md). Дополнительную информацию см. в разделе ["Расширения для определенных пользователем свойств"](news_p_artikelverwaltung_benutzerdefinierteeigenschaften_212382.md).

Дополнительные форматы для операции "ExportNCData"

Для экспорта данных ЧУ с помощью операции ExportNCData теперь доступны также [форматы](Glossary_o_formate.md) "ЧУ, медь", "DXF, медь" и "Изгиб шины".
...
Кроме того, в классе FunctionDefinition появилось новое свойство Identifiers, с помощью которого можно определить все буквенные [обозначения](Glossary_o_bezeichnungen.md) (для всех имеющихся стандартов).
...
Если в [сценарии](Glossary_o_scripte.md) нужно предоставить собственную операцию посредством атрибута [DeclareAction], теперь существует дополнительная возможность полностью использовать ActionCallingContext в качестве параметра метода обработки.

Определить координаты всех угловых точек для 3D-размещения изделий

Теперь в классе Placement3D есть новое свойство Corners. С помощью этого свойства можно определить абсолютные и относительные координаты всех угловых точек для 3D-[размещения изделий](Glossary_o_artikelplatzierungen.md).

Создать рамки макросов в виде ломаных линий

С помощью EPLAN API теперь также можно создавать [рамки макросов](Glossary_o_makrokaesten.md) в виде многоугольников. Определение ломаных линий выполняется при помощи свойства LogicalAreaSegments из класса MacroBox.
...
* При помощи значения selectDevice выбирается новое устройство для имеющихся объектов. При этом все данные устройств, включая данные ссылок изделий, теперь удаляются и заново предоставляются в соответствии с выбором [устройства](Glossary_o_betriebsmittel.md).
...
Теперь можно определить текущий выбор в навигаторе макросов с помощью EPLAN API. При помощи метода Selection из класса SelectionSet вы можете определять объекты типа MacroEntry, которые представляют выбранные [макросы](Glossary_o_makros.md).
...
С помощью нового метода ExportAddressOverview из класса PlcService теперь можно экспортировать [распределение адресного пространства](Glossary_o_adressraumbelegung.md) ЦПУ. В диалоговом окне Адреса / списки присвоения интерфейса пользователя это соответствует пункту меню Экспортировать распределение адресного пространства под кнопкой [Дополнительно] (путь меню Данные проекта > ПЛК > Адреса / списки присвоения).

Связка сегментов с 3D-размещениями изделий

В навигаторе предварительного планирования теперь можно связать [сегменты](Glossary_o_segmente.md) с 3D-размещениями изделий. В EPLAN API это можно сделать при помощи свойства PlanningSegment из класса Function3D.
