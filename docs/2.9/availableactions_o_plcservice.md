...
CONVERTERID |  Идентификатор для программ [конфигурации](Glossary_o_konfigurationen.md) ПЛК. Возможные значения:
...
IMPORTMATCH |  Параметры синхронизации при импорте данных ПЛК. Процесс импорта синхронизирует импортируемые [объекты](Glossary_o_objekte.md) с объектами, имеющимися в проекте. В зависимости от выбранного параметра поиск совпадений выполняется на основе внутренних [ид.](Glossary_o_id.md) объекта или идентифицирующих названий объектов. Если импортируемый объект подходит к имеющейся [функции](Glossary_o_funktionen.md), [свойства](Glossary_o_eigenschaften.md) имеющейся функции обновляются, а для объектов, не имеющих соответствия, в проекте генерируется новая функция. Возможные параметры:
...
CONFIGFILE |  Путь файла конфигурации для генерирования [схемы соединений](Glossary_o_anschlussbilder.md). Этот параметр оказывает влияние, только когда параметр TYPE имеет следующее значение: GENERATEPLCSCHEMATIC.
