...
Выберите в этом диалоговом окне / на этой вкладке словарь, который необходимо использовать для перевода. Словарь может основываться на базе данных Access или на базе данных сервера SQL. Все [настройки](Glossary_o_einstellungen.md) относятся к профилю определенного пользователя.
...
Кнопка ![](images/all_new_as.png){: .ui-icon } (Создать) рядом с полями Access и Сервер SQL дает вам возможность [создать](Glossary_o_erstellen.md) новый словарь.
...
* Перезаписать: EPLAN перезаписывает существующие ключевые слова и / или тексты переводов. Обратите внимание на то, что при этом ***все*** [языки перевода](Glossary_o_uebersetzungssprachen.md) в словаре перезаписываются, даже если в списке отсутствующих слов используются не все языки перевода.
...
!!! tip "Совет:"
    ...
    Если хотите, чтобы отображались все [варианты](Glossary_o_varianten.md) перевода для записи (Tacho-generator), выберите Всплывающее меню > Перевести или нажмите ++Ctrl++ + [L].
