...
Выберите из раскрывающегося списка [свойства](Glossary_o_eigenschaften.md), которые будут выведены в списке ниже: Все, Данные или Настройки. В списке выводятся все свойства выбранного типа, которым еще не было присвоено форматирование.
...
Если этот флажок установлен, то структурные идентификаторы скрываются, если они идентичны со структурными идентификаторами окружения (Обозначение местоположения или страницы), в котором они выводятся. Это также будет относиться к свойству ОУ (только структуры проекта) ([ид.](Glossary_o_id.md) 20020).
...
Если в поле Язык Вы выбираете запись "Все [языки отображения](Glossary_o_anzeigesprachen.md)", то это свойство считается пустым, если ***все*** языки отображения тоже имеют пустое значение. Если в поле Язык выбран определенный язык, то учитываться будут как этот, так и заданный в поле Альтернативный язык второй язык.
...
Если включен этот флажок, то [разделитель](Glossary_o_trennzeichen.md), расположенный ***перед*** текущим свойством, скрывается, если скрывается это свойство.. "Предыдущий разделитель" — это разделитель, который в списке Выбранные элементы формата находится перед текущим свойством.
...
!!! note "Замечание:"
    ...
    Тексты для числовых значений свойств внутренне переводятся на все доступные языки диалогового окна. Если в качестве языка отображения вы выбрали язык, который недоступен как [язык диалогового окна](Glossary_o_dialogsprache.md), текст будет отображен на языке, установленном как альтернативный язык.
...
Таблицы задаются с помощью [свойства проекта](Glossary_o_projekteigenschaften.md) Свойство блока: Текст замены [n] и свойства [формы](Glossary_o_formulare.md) Форматированные свойства: Текст замены [n]. При помощи индекса [n] можно определить до десяти таблиц.
...
* Одноязычн. (перемен.): если для свойства выбран этот параметр, переведенный текст отображается на языке, заданном для [настройки](Glossary_o_einstellungen.md) Одноязычн. (перемен.).
