...
При помощи файлов [конфигурации](Glossary_o_konfigurationen.md) можно влиять на экспорт и импорт данных из [базы данных изделий](Glossary_o_artikeldatenbanken.md). Файл конфигурации может при этом выбираться в диалоговых окнах Импортировать записи данных и Экспортировать записи данных (Сервисные программы > Изделие > Управление > [Дополнительно] > Импортировать и ... > Экспортировать) щелчком по ++...++ в поле Тип файла, если в качестве типа файла выбрана одна из записей "CSV для ++...++ ", "ECAD" или "Текст".
...
* Определите "преобразовать-регион-зависимый=1", чтобы при импорте использовалось представление чисел панели управления. Для [схемы](Glossary_o_schemata.md) района "Немецкий (Германия)" имеет значение, например, то, что точка "." используется для [обозначения](Glossary_o_bezeichnungen.md) тысяч, а запятая используется "," как [разделитель](Glossary_o_trennzeichen.md) разрядов (например, "1.200,00").
...
Цифра '.1' обозначает первую строку в таблице Произвольн. [свойства](Glossary_o_eigenschaften.md). Атрибуты обрабатываются аналогичным образом.
