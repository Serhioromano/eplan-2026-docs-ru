...
На сегмент переносятся [свойства](Glossary_o_eigenschaften.md) шаблона. При этом заполняются только пустые значения, уже имеющиеся значения свойства не переписываются. Дополнительно сегменту можно присвоить иные свойства или переписать скопированные из шаблона значения.
