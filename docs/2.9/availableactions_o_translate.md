...
Это имя используется как формат вывода для списка отсутствующих слов. Возможные [форматы](Glossary_o_formate.md) вывода:
...
PAGEFILTERNAME |  Имя фильтра страницы. Переводятся только те страницы, которые прошли через [фильтр](Glossary_o_filter.md) с именем PAGEFILTERNAME. Этот параметр оказывает влияние, только когда параметр TYPE имеет следующее значение: TRANSLATEPAGES.
