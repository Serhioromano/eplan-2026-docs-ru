...
Импорт и экспорт [базы данных изделий](Glossary_o_artikeldatenbanken.md) происходит при помощи языка XML. Данные в соответствии с этим языком должны быть структурированы следующим образом:

![](images/xmlexport_example_av.png)

!!! note "Замечание:"

    Тег <partsmanagement> можно определить только ***один*** раз! Все остальные теги могут быть определены любое количество раз. Их определение также можно опустить, например, если необходимо задать шаблон [функции](Glossary_o_funktionen.md) или схему сверления без указания данных.
    ***Исключение:***
