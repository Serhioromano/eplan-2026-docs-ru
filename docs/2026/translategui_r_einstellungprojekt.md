...
!!! example "Пример:"
    ...
    Существует подходящий словарь с ключевыми словами на исходном языке `en_US` и соответствующие тексты перевода на языках `fr_FR`, `zh_CN`, `ru_RU`. Заданы следующие настройки:Настройка Язык Языки переводаen_US, fr_FR, zh_CN, ru_RUИсходный языкen_USЯзыки отображенияzh_CN, ru_RUТексты проекта вы вводите на английском, так как в качестве исходного языка выбран `en_US`, и переводите проект. Все тексты проекта переводятся на языки `fr_FR`, `zh_CN` и `ru_RU`. На `en_US` они уже существуют.Графический редактор:Здесь тексты проектов отображаются исключительно на языках `zh_CN` и `ru_RU`.Диалоговые окна:Здесь отображаются переводимые тексты проектов, например описания страниц в навигаторе страниц, на исходном языке проекта, т. е. `en_US`.
...
!!! example "Пример:"
    ...
    запись `Cables w. Terminals. Circ.-Breaker and more.` будет выведена в списке отсутствующих слов следующими восемью сегментами перевода:1-й сегмент перевода: `Cables` 2. сегмент перевода: `w` 3. сегмент перевода: `Terminals` 4-й сегмент перевода: `-Breaker` 5-й сегмент перевода: `.` 6-й сегмент перевода: `Circ` 7-й сегмент перевода: `more` 8-й сегмент перевода: `and`
...
!!! example "Пример:"
    ...
    запись `Cables w. Terminals. Circ.-Breaker and more.` будет выведена в списке отсутствующих слов следующими тремя сегментами перевода:1-й сегмент перевода: `Cables w` 2. сегмент перевода: `Terminals` 3. сегмент перевода: `Circ.-Breaker and more`
...
!!! example "Пример:"
    ...
    В поле **Сегмент** выбрана настройка **Слово**, и на страницесхемы соединенийв виде текста введена следующая комбинация слов на русском языке: `Not-Aus` Поле **Дополнительные разделители слов** не заполнено.На основе пустого словаря сгенерируйте список отсутствующих слов. Eplan выведет сегмент перевода `Not-Aus`.Теперь введите в поле **Дополнительные разделители слов** символ "-" (дефис). Необходимо провести повторную генерацию списка отсутствующих слов. Теперь Eplan выводит следующие 3 сегмента перевода в алфавитном порядке: `-``Aus``Not`Если дополнительныйразделительслов был введен в словаре в список "Слова не переводить", то он не будет выведен в списке отсутствующих слов. Если дополнительный разделитель слов не внесен в этот список, то он будет выведен в списке отсутствующих слов.
