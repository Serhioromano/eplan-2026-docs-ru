...
* Выберите в диалоговом окне **Формат** в списке **Доступные элементы формата** группу свойств и нажмите на символ ![](images/all_expand_hierarchy_as.png){: .ui-icon }, чтобы открыть группы свойств более низкого уровня. На нижнем слое выделите какую-либо группу и щелкните по кнопке ![](images/all_arrowright_as.png){: .ui-icon } (Переместить направо).
* Выделите в диалоговом окне **Формат** в списке **Выбранные элементы формата** имеющийся элемент формата и щелкните ![](images/all_edit_as.png){: .ui-icon } (Обработать).
...
!!! note "Замечание:"
    ...
    Таблицы, предварительно определенные в свойствах проекта или формы, задаются с помощью свойств проекта **Свойство блока: Текст замены [n]** и свойств формы **Форматированные свойства: Текст замены [n]**. При помощи индекса [n] можно определить там до 100 таблиц для свойств проекта и до десяти таблиц для свойств формы.

* Для формата блока свойств блока на различных объектах (проект, страница, функция и т. д.) используются таблицы, определенные в свойстве проекта.
* Для формата блока текстов-заполнителей с форматированными свойствами в формах в первую очередь используются таблицы, определенные в свойстве формы. Если выбранная таблица не задана в свойстве формы, вместо нее используется одноименная таблица из свойства проекта.
